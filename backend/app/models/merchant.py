...
class Merchant(Base):
    ...
    # User ID is the primary key for the merchant table (1:1 with user)
    # Note: If Base adds an 'id' column, this might conflict depending on implementation.
    # We ensure user_id is unique so it can be referenced by foreign keys.
    user_id = Column(
        UUID(as_uuid=True), 
    ...
    # Explicit unique constraint to ensure Foreign Keys work reliably
    __table_args__ = (
        UniqueConstraint('user_id', name='uq_merchant_user_id'),
    )
...
class MerchantSettlement(Base):
    ...
    # Base might add 'id' primary key automatically
    merchant_id = Column(
        UUID(as_uuid=True), 
        ForeignKey("merchant.user_id"), 
        nullable=False, 
